...
class Metric(object):
  ...
  Attributes:
    ...
    name_tmpl: Used by Metrics that have children. It's applied to children's
      names in the output.
    enable_caching: If this Metric class will be cached in computation across
      ...
      have the feature enabled. For custom Metrics, different instances don't
      share the same result in cache, because we don't know what attributes
      matter. If you want to enable caching across instances for a custom
      Metric, make sure you read the 'Custom Metric' and 'Caching' sections in
      the demo notebook and understand the `additional_fingerprint_attrs`
      attribute before setting enable_caching to True.
  ...
  def get_fingerprint(self):
    ...
    # custom Metrics with enable_caching. Otherwise different instances of the
    # same class are always saved under different keys.
    if type(self).__name__ not in BUILT_INS and not self.enable_caching:
