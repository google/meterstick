...
'''  # You can overwrite this from outside w/ custom CSS.
# Line breaker in a flex display.
_LINE_BREAK = '<div class="ci-display-flex-line-break"></div>'
...
def _sorted_long_to_wide(df, dims, sort_by):
  ...
  Args:
    df: A dataframe in long format, could just be metrics_types.as_dataframe().
      Example screenshot/Y4BWcHWh5z6
  ...
  df = df.pivot_table(
      index=existing_index_cols, columns='Metric', values=val_cols)
  ...
  # val_col might be dropped during pivot b/c of na, so we make a dict first.
...
def _pre_aggregate_df(df,
  ...
  If aggregate_dimensions, all dimension columns are collected into a Dimensions
  column, and original columns are dropped.
  ...
  Returns:
    A pandas dataframe with stylized content for display.
...
class MetricFormatter(object):
  """A custom formatter for metric columns which has similar display to Vista.

  Concatenates 'Value', 'Ratio', 'CI_Lower', 'CI_Upper' columns in df to a
  stylized form which can be rendered to HTML directly later. Cells w/
  positive CI change are rendered green, w/ negative CI change are rendered
  red.

  Attributes:
    metric_formats: A dict specifiying how to display metric values. Keys can be
      ...
      screenshot/NTghj5F49wj for an example.
  ...
  Returns:
    A string specifiying a named <div> containing concatenated values. Div may
      include html classes used to style with CSS.
  """

  def __init__(self, metric_formats=None, if_flip_color=None):
  ...
  def __call__(self, x, div=_div, span=_span, line_break_join=_LINE_BREAK.join):
    ...
    Returns:
      A string specifiying a named <div> containing dimension. The returned div
    ...
    if ((ci_lower > 0 and not self.if_flip_color) or
      ...
      return div(res, 'ci-display-good-ci-change ci-display-cell')
    if ((ci_upper < 0 and not self.if_flip_color) or
        (ci_lower > 0 and self.if_flip_color)):
      return div(res, 'ci-display-bad-ci-change ci-display-cell')
...
def dimension_formatter(x,
                        div=_div,
                        span=_span,
                        line_break_join=_LINE_BREAK.join):
...
def _metric_formatter(df, dims, if_flip_colors, metric_formats=None):
...
def _add_is_control_and_control_id(df, ctrl_id):
  """Add Is_Control and Control_Id columns to df."""
  if not isinstance(ctrl_id, dict):
    ...
    for ctrl, exp_ids in six.iteritems(ctrl_id):
      if not isinstance(exp_ids, list):
        raise ValueError(
            'The experiment id(s) {} is not a list.'.format(exp_ids))
      for e in exp_ids:
...
def get_formatted_df(df,
  ...
  Args:
    ...
    description: The column name for the optional description of each row. It'll
      be rendered with experiment id for a more meaningful display.
  ...
  custom_formatters = _metric_formatter(formatted_df, dims, if_flip_colors,
                                        metric_formats)
...
def display_formatted_df(formatted_df):
  ...
  pd.set_option('display.max_colwidth', -1)  # So HTML won't get truncated.
