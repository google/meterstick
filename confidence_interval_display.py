...
class MetricFormatter(object):
  ...
  Attributes:
    metric_formats: A dict specifying how to display metric values. Keys can be
      'Value' and 'Ratio'. Values can be 'absolute', 'percent', 'pp', 'int' or a
  ...
  def __init__(
    ...
    default_value_format = 'int' if is_integer else 'absolute'
  ...
  def _format_value(self, val, form, is_ci=False):
    ...
    Args:
      ...
      form: 'Absolute', 'percent', 'pp', 'int' or a formatting string.
...
def _is_integer_column(col_series):
  """Checks if all non-null values in a metric column are whole numbers."""
  vals = []
  for x in col_series:
...
def _get_formatter(df,
  ...
  Args:
    ...
    metric_formats: A dict specifying how to display metric values. Can be a
      flat dict applying globally to all metrics (e.g. {'Value': '{:,.0f}',
      'Ratio': '{:.1f}%'}). Keys can be 'Value' and 'Ratio'. Values can be
      'absolute', 'percent', 'pp', or a formatting string (e.g., '{:.2%}'). By
      default, Value is in absolute form (or integer if all values are whole
      numbers) and Ratio in percent. Can also be a nested dict keyed by metric
      column name to specify different formats for each metric (with an optional
      'default' key for fallback), e.g. {'Clicks': {'Value': '{:,.0f}', 'Ratio':
      '{:.1f}%'}, 'Latency': '{:.2f}%', 'default': {'Value': 'absolute',
  ...
  for i, col in enumerate(df.columns):
    if col in dims + ['Experiment_Id', 'Is_Control', 'Description']:
      ...
      is_int = _is_integer_column(df[col])
