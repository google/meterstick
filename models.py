  ...
  def compute_through_sql(self, table, split_by, execute, mode):
    try:
      if (
        ...
        print(
            'INFO: SQL generation for your Model can be slow because of the'
            ' number of features. Try compute_on_sql(mode="mixed") (for small'
...
class Ridge(Model):
  ...
  def get_sql_and_with_clause(self, table, split_by, global_filter, indexes,
    ...
    if self.normalize:
      raise NotImplementedError(
          'SQL generator is not implemented for models with normalization.'
      )
    ...
    if isinstance(self, Ridge) and self.alpha:
      ...
      x_t_x += self.alpha / sympy.Symbol(n_obs) * penalty
...
def get_data(
  ...
  data, with_data = m.children[0].get_sql_and_with_clause(
      table,
