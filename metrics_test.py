...
class TestCaching(parameterized.TestCase):
  ...
  def test_caching_disabled_for_custom_metric_by_default(self):
    ...
    output = metrics.MetricList([Custom(1), Custom(2)]).compute_on(self.df)
    expected = pd.DataFrame([[1, 2]], columns=['Foo'] * 2)
    testing.assert_frame_equal(output, expected)

  def test_caching_enabled_for_custom_metric(self):
