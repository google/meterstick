...
"""Tests for meterstick.v2.metrics."""
...
class JackknifeTests(unittest.TestCase):
  ...
  def test_internal_caching_split_by(self):
    df = pd.DataFrame({
        'X': range(12),
  ...
  def test_internal_caching_with_operation_split_by(self):
    df = pd.DataFrame({
        'X': np.arange(0, 6, 0.5),
        'cookie': [1, 2, 3, 1, 2, 3] * 2,
        'grp': list('AABBAB') * 2,
