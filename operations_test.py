...
OPERATIONS = [
    ...
    (
        'CUPED',
        operations.CUPED(
            'condition', 0, covariates=metrics.Mean('X'), stratified_by='grp'
        ),
    ),
    (
        'PrePost',
...
class DistributionTests(absltest.TestCase):
  ...
  def test_distribution(self):
    ...
    expected = pd.DataFrame(
        {'Distribution of sum(X)': [0.25, 0.75]}, index=['A', 'B']
    )
  ...
  def test_distribution_over_multiple_columns(self):
    ...
    expected = pd.DataFrame({
        'Distribution of sum(X)': [1.0, 0.5, 0.25, 0.25],
        'country': ['EU', 'US', 'US', 'US'],
        'grp': ['B', 'A', 'A', 'B'],
  ...
  def test_distribution_splitby(self):
    ...
    expected = pd.DataFrame({
        'Distribution of sum(X)': [1.0, 2.0 / 3, 1.0 / 3],
        'grp': ['B', 'A', 'B'],
        'country': ['EU', 'US', 'US'],
    })
  ...
  def test_distribution_pipeline(self):
    output = (
        self.sum_x
        | operations.Distribution('grp')
        | metrics.compute_on(self.df)
    )
    expected = pd.DataFrame(
        {'Distribution of sum(X)': [0.25, 0.75]}, index=['A', 'B']
    )
...
class CumulativeDistributionTests(absltest.TestCase):
  ...
  def test_cumulative_distribution_over_multiple_columns(self):
    ...
    expected = pd.DataFrame({
        'Cumulative Distribution of sum(X)': [1.0, 0.5, 0.75, 1],
        'country': ['EU', 'US', 'US', 'US'],
        'grp': ['B', 'A', 'A', 'B'],
        'platform': ['mobile', 'desktop', 'mobile', 'desktop'],
    })
  ...
  def test_cumulative_distribution_splitby(self):
    ...
    expected = pd.DataFrame({
        'Cumulative Distribution of sum(X)': [1.0, 1.0 / 3, 1.0],
        'grp': ['A', 'A', 'B'],
        'country': ['EU', 'US', 'US'],
    })
    expected.set_index(['country', 'grp'], inplace=True)
  ...
  def test_cumulative_distribution_order_splitby(self):
    ...
    expected = pd.DataFrame({
        'Cumulative Distribution of sum(X)': [1.0, 2.0 / 3, 1.0],
        'grp': ['A', 'B', 'A'],
        'country': ['EU', 'US', 'US'],
    })
    expected.set_index(['country', 'grp'], inplace=True)
  ...
  def test_cumulative_distribution_multiple_metrics(self):
    ...
    expected = pd.DataFrame(
        {
            'Cumulative Distribution of sum(X)': [0.75, 1.0],
            'Cumulative Distribution of count(X)': [0.5, 1.0],
        },
        index=['A', 'B'],
        columns=[
  ...
  def test_cumulative_distribution_pipeline(self):
    output = (
        self.sum_x
        | operations.CumulativeDistribution('grp')
        | metrics.compute_on(self.df)
    )
    expected = pd.DataFrame(
        {'Cumulative Distribution of sum(X)': [0.75, 1.0]}, index=['A', 'B']
...
class PercentChangeTests(absltest.TestCase):
  ...
  def test_percent_change_melted_include_baseline(self):
    ...
    output = metric.compute_on(self.df, melted=True)
    expected = pd.DataFrame({
        'Value': [0.0, 150.0, 0.0, 0.0],
        'Metric': [
            'sum(X) Percent Change',
            'sum(X) Percent Change',
            'count(X) Percent Change',
            'count(X) Percent Change',
        ],
        'Condition': [0, 1, 0, 1],
    })
  ...
  def test_percent_change_splitby(self):
    ...
    expected = pd.DataFrame(
        {
            'sum(X) Percent Change': [0.0, 100.0 / 3, 0.0, 200.0 / 3, np.nan],
            'count(X) Percent Change': [0.0, -50.0, 0.0, 0.0, np.nan],
            'Condition': [0, 1, 0, 1, 1],
            'grp': ['A', 'A', 'B', 'B', 'C'],
        },
        columns=[
  ...
  def test_percent_change_multiple_condition_columns(self):
    ...
    metric = operations.PercentChange(
        ['Condition', 'grp'], (0, 'A'), self.metric_lst
    )
    output = metric.compute_on(df)
    df['Condition_and_grp'] = df[['Condition', 'grp']].apply(tuple, 1)
    expected_metric = operations.PercentChange(
        'Condition_and_grp', (0, 'A'), self.metric_lst
    )
  ...
  def test_percent_change_multiple_condition_columns_include_baseline(self):
    df = self.df.copy()
    metric = operations.PercentChange(
        ['Condition', 'grp'], (0, 'A'), self.metric_lst, True
    )
    output = metric.compute_on(df)
    df['Condition_and_grp'] = df[['Condition', 'grp']].apply(tuple, 1)
    expected_metric = operations.PercentChange(
        'Condition_and_grp', (0, 'A'), self.metric_lst, True
    )
    expected = expected_metric.compute_on(df)
    expected = pd.DataFrame(
        expected.values, index=output.index, columns=output.columns
...
class AbsoluteChangeTests(absltest.TestCase):
  ...
  def test_absolute_change_splitby(self):
    ...
    expected = pd.DataFrame(
        {
            'sum(X) Absolute Change': [0.0, 1.0, 0.0, 2.0, np.nan],
            'count(X) Absolute Change': [0.0, -1.0, 0.0, 0.0, np.nan],
            'Condition': [0, 1, 0, 1, 1],
            'grp': ['A', 'A', 'B', 'B', 'C'],
        },
        columns=[
  ...
  def test_absolute_change_splitby_melted(self):
    ...
    output = metric.compute_on(self.df, 'grp', melted=True)
    expected = pd.DataFrame({
        'Value': [0.0, 1.0, 0.0, 2.0, np.nan, 0.0, -1.0, 0.0, 0.0, np.nan],
  ...
  def test_absolute_change_multiple_condition_columns(self):
    ...
    output = metric.compute_on(df)
    df['Condition_and_grp'] = df[['Condition', 'grp']].apply(tuple, 1)
    expected_metric = operations.AbsoluteChange(
        'Condition_and_grp', (0, 'A'), self.metric_lst
    )
    expected = expected_metric.compute_on(df)
    expected = pd.DataFrame(
...
class PrePostChangeTests(absltest.TestCase):
  ...
  def test_where(self):
    ...
    metric_no_filter = operations.PrePostChange(
        'condition', 0, self.sum_x, self.sum_prex, 'cookie'
    )
    output = metric.compute_on(self.df)
  ...
  def test_with_jackknife(self):
    ...
    m = operations.PrePostChange(
        'condition', 0, self.sum_x, self.sum_prex, 'grp2'
    )
  ...
  def test_display(self):
    ...
    prepost = operations.PrePostChange(
        'condition', 0, self.sum_x, self.sum_prex, 'grp'
    )
...
class CUPEDTests(absltest.TestCase):
  ...
  def test_where(self):
    ...
    metric_no_filter = operations.CUPED(
        'condition', 0, self.sum_x, self.sum_prex, 'cookie'
    )
    output = metric.compute_on(self.df)
    expected = metric_no_filter.compute_on(self.df[self.df.grp == 'A'])
...
class MHTests(absltest.TestCase):
  ...
  def test_mh_melted_include_baseline(self):
    ...
    expected = expected = pd.DataFrame({
        'Value': [0.0, 40.0, 0.0, 40.0],
        'Metric': [
            'sum(Y) / sum(X) MH Ratio',
            'sum(Y) / sum(X) MH Ratio',
            'cvr MH Ratio',
            'cvr MH Ratio',
        ],
  ...
  def test_mh_splitby_melted(self):
    ...
    output.sort_index(
        level=['Metric', 'grp'], ascending=[False, True], inplace=True
    )  # For Py2
    expected = pd.DataFrame({
        'Value': [0.0, 40.0, 0.0, 80.0, 0.0, 40.0, 0.0, 80.0],
  ...
  def test_mh_multiple_condition_columns(self):
    ...
    metric = operations.MH(
        ['Condition', 'grp'], (0, 'A'), 'Id', self.metric_lst
    )
    output = metric.compute_on(df)
    df['Condition_and_grp'] = df[['Condition', 'grp']].apply(tuple, 1)
    expected_metric = operations.MH(
        'Condition_and_grp', (0, 'A'), 'Id', self.metric_lst
    )
    expected = expected_metric.compute_on(df)
    expected = pd.DataFrame(
        expected.values, index=output.index, columns=output.columns
  ...
  def test_mh_multiple_condition_columns_include_baseline(self):
    ...
    metric = operations.MH(
        ['Condition', 'grp'], (0, 'A'), 'Id', self.metric_lst, True
    )
    output = metric.compute_on(df)
    df['Condition_and_grp'] = df[['Condition', 'grp']].apply(tuple, 1)
    expected_metric = operations.MH(
        'Condition_and_grp', (0, 'A'), 'Id', self.metric_lst, True
    )
    expected = expected_metric.compute_on(df)
    expected = pd.DataFrame(
        expected.values, index=output.index, columns=output.columns
...
PRECOMPUTABLE_OPERATIONS = [
    ...
    (
        ...
        models.LinearRegression(
            metrics.Sum('X'), metrics.Mean('Y'), 'condition'
        ),
    ),
...
class BootstrapTests(parameterized.TestCase):
  ...
  def test_bootstrap_unit(self, m):
    ...
    bootstrap_unit_no_opt = operations.Bootstrap(
        'unit', m, 5, enable_preaggregation=False
    )
    np.random.seed(42)
    with mock.patch.object(
...
OPERATIONS = [
    ...
    (
        'CUPED',
        operations.CUPED(
            'condition', 0, metrics.Sum('X'), metrics.Sum('Y'), 'grp'
        ),
    ),
    (
        'PrePostChange',
...
class FilterTest(parameterized.TestCase):
  ...
  def test_metriclist(self, op):
    ...
    m = metrics.MetricList(
        (
            metrics.Ratio('Y', 'X', where='X > 5'),
            metrics.Ratio('X', 'Y', where='X > 5'),
        ),
        where='Y > 4',
    )
    metric = op(m)
...
class CachingTest(parameterized.TestCase):
  ...
  def test_leaf_caching(self, op):
    leaf = metrics.MetricList(
        [metrics.Ratio('Y', 'X'), metrics.Ratio('X', 'Y')]
    )
    if not op.children:
      op = op(leaf)
    SUM_COMPUTE_THROUGH.mock.reset_mock()
