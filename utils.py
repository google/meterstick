...
def get_ridge_coefficients(
  ...
  Args:
    x_t_x_elements: The SQL column names of the elements of X'X. It's the 1st
    ...
    normalize: If the model normalizes x. It only has effect when fit_intercept
      is also True. If normalize, all the AVG(x) columns are 0.
  ...
  del normalize
  # normalize = fit_intercept and normalize
  if fit_intercept:
    x_t_y_elements = ['1'] + x_t_y_elements
  n = len(x_t_y_elements)
  if n > 5:
    print(
        f'WARNING: Doing symbolic computation on {n}*{n} matrix. It will be'
        " slow. Consider using compute_on_sql(mode='mixed')."
    )
  ...
  coefs = x_t_x_inv * x_t_y
  if n < 5:
    coefs = sympy.simplify(coefs)  # slow if n is large
  return coefs
